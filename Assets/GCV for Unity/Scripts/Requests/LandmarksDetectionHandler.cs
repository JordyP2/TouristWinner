...
public class LandmarksDetectionHandler
{
	...
	public static IEnumerator Run(Action<List<Vertices[]>, List<string[]>, string> onSuccess, Action<string> onFailure, string image_path, bool isUrl)
    {
        ...
        if (RequestHandler.CheckResult())
        {
            try
            {
                LandmarksDetection res = (LandmarksDetection)JsonFormatter.DeserializeObject(RequestHandler.GetResult(), typeof(LandmarksDetection));

                List<Vertices[]> borders = new List<Vertices[]>();

                foreach (var border in res.responses[0].landmarkAnnotations)
                    borders.Add(border.boundingPoly.vertices);

                List<string[]> values = new List<string[]>();

                foreach (var v in res.responses[0].landmarkAnnotations)
                {
